...
const procMsg = async (raw, socket, store) => {
  ...
  if (mContent.documentWithCaptionMessage)
    mContent = mContent.documentWithCaptionMessage.message;
  ...
  if (contextInfo?.quotedMessage) {
    ...
    quoted = {
      key: {
        remoteJid: chat,
        fromMe: qSender === jidNormalizedUser(socket.user.id),
  ...
  return {
    ...
    adReply: async (text, title = "", options = {}) => {
      return await socket.sendMessage(
        chat,
        {
          ...
          contextInfo: {
            externalAdReply: {
              title: title || config.botName || "Ryo Bot",
              body: title
                ? ""
    ...
    react: async (emoji) =>
      socket.sendMessage(chat, { react: { text: emoji, key } }),
